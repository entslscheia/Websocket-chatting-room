...
<head>
    ...
    <style type="text/css">
        ...
        #us p:hover,#us p:active,#us p.ck{background-color:#069; color:#FFF;} /* 显示大家*/
        #us p.my:hover,#us p.my:active,#us p.my{color:#333;background-color:transparent;}/* 显示自己的昵称*/
...
(function(){
   ...
   // var url='ws://www.yxsss.com:8000';
    ...
    function st(){//////////////////////////////////
        ...
        so.onmessage=function(msg){
           ...
           // alert(da.users[1]['name']);
            ...
            if(da.type== "add"){//有其他新用户加入
                //alert("add");///////////////////////////////////////////////
                var obj=A.$$('<p>'+da.name+'</p>');//用于在右侧列表显示用户名
                lus.appendChild(obj);
                cuser(obj,da.code,0);
                obj=A.$$('<p><span>['+da.time+']</span>欢迎<a class="admin'+da.isadmin+'">'+da.name+'</a>加入</p>');//isadmin????
                //obj=A.$$('<p><span>['+da.time+']</span>欢迎<a class="admin'+'">'+da.name+'</a>加入</p>');
                c=da.code;
            }
            else if(da.type=='madd'){//自己加入
                mkey=da.code;//my key的获取
               // alter(da.users);
                da.users.unshift({'code':'all','name':'大家'});//把新的二元组(code,name):(all,大家)加入users中
				//alter(123);
                for(var i=0;i<da.users.length;i++){//从服务器端获取所有的users列表
            ...
            if(obj==false){//说明不是有人加入，而是其他类型的消息
                if(da.type=='rmove'){
                    if(da.msg){//
                        var _tms = da.msg;
                    }
                    else{
                        var _tms = users[da.code].innerHTML+'退出聊天室';
                    }
                    var obj=A.$$('<p class="c2"><span>['+da.time+']</span>'+_tms+'</p>');
                    lct.appendChild(obj);
                    users[da.code].del();
                    delete users[da.code];//将该用户在列表中删除
                }
                else if(da.type=='mes'){//发送消息的处理
                    ...
                    //da.code 发信息人的code
                    if(da.code1==mkey){//code1标记发送目标的code,即如果接收方是我
                        obj=A.$$('<p class="c3"><span>['+da.time+']</span><a>'+users[da.code].innerHTML+'</a>对我说：<var class="admin'+da.isadmin+'">'+da.nrong+'</var></p>');
                        c=da.code;
                    }
                    else if(da.code==mkey){//code标记发送源头的code,即如果发送方是我
                        if(da.code1!='all')
                            obj=A.$$('<p class="c3"><span>['+da.time+']</span>我对<a>'+users[da.code1].innerHTML+'</a>说：<var class="admin'+da.isadmin+'">'+da.nrong+'</var></p>');
                        else
                            obj=A.$$('<p><span>['+da.time+']</span>我对<a>'+users[da.code1].innerHTML+'</a>说：<var class="admin'+da.isadmin+'">'+da.nrong+'</var></p>');
                        c=da.code1;
                    }
                    else{
                   	 	obj=A.$$('<p><span>['+da.time+']</span><a>'+users[da.code].innerHTML+'</a>对'+users[da.code1].innerHTML+'说：<var class="admin'+0+'">'+da.nrong+'</var></p>');
                     	c=da.code;
                    }
    ...
    A.$('sd').onclick=function(){//点击发送消息后的处理
    ...
    function cuser(t,code,isa){//it's critical
        users[code]=t;//将从服务器端get到的users列表存入本地
       // if(isa){
            t.title = '双击剔除此用户!';
        //}
        t.ondblclick=function(){//双击鼠标的操作
            so.send('type=rmove&key='+code+'&name='+t.innerHTML);
        }
        t.onclick=function(){
            t.parentNode.children.rcss('ck','');
            t.rcss('','ck');
            key=code;//实现对某一个用户说话的功能
        }
    }
    ...
    img.onload=function(){
        ...
        var da=rc.toDataURL();
        so.send('type=pic&nr='+esc(da)+'&key='+key);//发送图片
    }
